...
def calculate_max_loan(monthly_income, existing_debt, loan_term_years):
    ...
    loan_amount = available_payment * ((1 - (1 + monthly_interest_rate) ** -loan_term_months) / monthly_interest_rate)

    return round(loan_amount), round(available_payment), "Loan amount calculated successfully"
...
def handle_http_request(request):
    ...
    max_loan, monthly_payment, message = calculate_max_loan(monthly_income, existing_debt, loan_term_years)
    return jsonify(
        {
...
if __name__ == "__main__":
    ...
    max_loan, monthly_payment = calculate_max_loan(monthly_income, existing_debt, loan_term_years)
